...
def movies() :
# Movies
    with open('C:/Users/SDG/Documents/Github/3rd_pj_django/Neflex/movies.csv','r', newline='', encoding='latin_1') as csvfile:	
        data_reader = csv.DictReader(csvfile)

        for row in data_reader:
            # print(row)
            Movies.objects.create(
                id = row['imdb_movie_Id'],
                title = row['title'],
                genres = row['genres']
            )
    return print("movies 완료")


def ratings() :
    # ratings
    with open('C:/Users/SDG/Documents/Github/3rd_pj_django/Neflex/ratings.csv', newline='', encoding='utf-8-sig') as csvfile:	
        data_reader = csv.DictReader(csvfile)
        ...
        for row in data_reader:
            ...
            Ratings.objects.create(	
                ...
                user_Id = row['user_Id'],
                rating = row['rating'],
                imdb_movie_Id = int(row['imdb_movie_Id'])
                
            )   
    return print("ratings 완료")     
# movies()
ratings()
